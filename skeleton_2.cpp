...
namespace cfg {
    ...
    constexpr double kMinClusterR2 = 0.5;
...
static void handleOwnFill(InstrumentState& st, const std::string& sender, const std::string& orderId,
    ...
    if (st.role == Role::PIGGYBACK) {
        ...
        // pgBidId/pgAskId are cleared on the *next* requote (see
        // onPiggybackCenterChanged), not on fill, since a partial fill
        // doesn't mean the resting order is gone.
        if (orderId == st.pgBidId) {
            st.position += vol;
            st.realizedPnl -= static_cast<double>(vol) * static_cast<double>(price);
            PnlFile << nowNs() << "," << st.feed << ",piggyback_bid_fill," << orderId << "," << vol
                    << "," << price << "," << st.position << "\n";
        } else if (orderId == st.pgAskId) {
            st.position -= vol;
            st.realizedPnl += static_cast<double>(vol) * static_cast<double>(price);
            PnlFile << nowNs() << "," << st.feed << ",piggyback_ask_fill," << orderId << "," << vol
                    << "," << price << "," << st.position << "\n";
        }
        return;
    }
...
static void onPiggybackCenterChanged(InstrumentState& pst) {
    // Cancel existing resting quotes, if any. Uses PGYB_BID_CANCEL/
    // PGYB_ASK_CANCEL (distinct from the driver's ENTRY_CANCEL/EXIT_CANCEL)
    // so handleOrderReply/handleOwnFill never mistake a piggyback ack for a
    // driver entry/exit event.
    if (!pst.pgBidId.empty()) {
        requestCancel(pst, pst.pgBidId, ReqKind::PGYB_BID_CANCEL);
        pst.pgBidId.clear();
    }
    if (!pst.pgAskId.empty()) {
        requestCancel(pst, pst.pgAskId, ReqKind::PGYB_ASK_CANCEL);
        pst.pgAskId.clear();
    }

    // §5.1: one tick inside live best bid/ask; fall back to cEst +/- 1 if a
    // side of the book is currently empty.
    long long tick = pst.meta.ticksize > 0 ? pst.meta.ticksize : 1;
    long long bidPx = (pst.bidPx >= 0) ? *pst.cEst - (3*tick)
                                        : static_cast<long long>(std::llround(*pst.cEst)) - tick;
    long long askPx = (pst.askPx >= 0) ? *pst.cEst + tick
                                        : static_cast<long long>(std::llround(*pst.cEst)) + tick;
    bidPx = clampToBand(bidPx, pst.meta);
    askPx = clampToBand(askPx, pst.meta);

    // Queued behind the two cancels above via the same awaitingAck gate —
    // requestAdd already handles "queue if busy" so no extra logic needed
    // here, just call in the order we want them to eventually fire.
    requestAdd(pst, 'B', bidPx, kPiggybackSize, ReqKind::PGYB_BID_ADD);
    requestAdd(pst, 'S', askPx, kPiggybackSize, ReqKind::PGYB_ASK_ADD);

    DiscoveryLog << nowNs() << ",piggyback_requote," << pst.feed << ",cEst=" << *pst.cEst
                 << " bid=" << bidPx << " ask=" << askPx << "\n";
}
